...
Request
  │
  ▼
CORS (restricted to `CORS_ORIGIN` env var; defaults to localhost:5173/4173 in development)
  │
  ▼
Compression (gzip)
  │
...
| **Route protection (client)**        | `requireAuth()` in every `clientLoader` — calls `GET /api/auth/me`; throws `redirect('/login')` on 401                                                                                                                    |
