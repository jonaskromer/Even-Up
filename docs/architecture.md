...
│            activity, email, exchangeRate,       │
│            geminiReceipt (retry + fallback)      │
│  Error handler: ZodError, HttpError, Prisma     │
│                 ↕ JWKS              ↕ HTTPS      │
│         Supabase Auth (Cloud)   Google Gemini    │
│                                  (receipt OCR,    │
│                                  image discarded  │
│                                  after response)  │
└────────────────────┬────────────────────────────┘
...
│              PostgreSQL 16                      │
│                                                 │
│  12 tables, integer cents for all monetary vals │
│  Cascade deletes on group → expenses/members/   │
│  join-requests/receipt line items                │
...
├── groups.$groupId_.receipt.tsx                   # /groups/:groupId/receipt?expenseId=… (upload/review/edit)
...
│   ├── groups.$groupId_.expenses.$expenseId.edit → AddExpenseForm (banner slot links to
│   │                       │   "Edit line items" when the expense has receipt line items)
│   ├── groups.$groupId_.receipt → state machine, no URL change between steps:
│   │                       ├── 'upload'     → ReceiptUploadStep (camera/library input)
│   │                       ├── 'processing' → ReceiptProcessingStep (live retry/fallback
│   │                       │                   progress via NDJSON stream)
│   │                       ├── 'review'     → ReceiptLineItemReview (per-item
│   │                       │                   equal/exact/percent/shares, exclude toggle,
│   │                       │                   live per-member totals)
│   │                       └── 'confirm'    → AddExpenseForm (defaults from computed splits)
...
| **Receipt image handling**           | MIME type + 10MB size validated before any Gemini call; the image is base64-encoded in memory for the single outbound request and never written to disk or the database — see [ADR 012](adr/012-receipt-ai-parsing.md)   |
